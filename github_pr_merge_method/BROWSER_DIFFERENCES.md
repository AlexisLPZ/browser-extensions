...
**Firefox:** Requires an add-on ID and data collection declaration:
...
- Firefox requires a unique identifier for all Manifest V3 extensions. The UUID format is used here.
- `data_collection_permissions.required: ["none"]` explicitly declares that this extension does not collect or transmit any personal data. This is a Mozilla requirement (as of November 2025) for transparency about data collection practices.
- Valid values for `required` include: `"none"` (no data collection), `"interaction"`, `"web_activity"`, `"location"`, etc.
- See [Mozilla's data collection consent documentation](https://extensionworkshop.com/documentation/develop/firefox-builtin-data-consent/) for details.
